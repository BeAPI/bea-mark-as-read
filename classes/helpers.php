...
class Helpers {
	...
	public static function locate_template( $tpl ) {
		...
		$path = apply_filters( 'BEA/Helpers/locate_template/templates', array( 'views/' . BEA_MAS_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( BEA_MAS_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( BEA_MAS_DIR . 'views/' . $tpl . '.php' );
		}
